...
sub analysis {
    my ( $self, $arg ) = @_;
    if ( defined($arg) ) {
        $self->throw("[$arg] is not a Bio::EnsEMBL::Analysis object") unless $arg->isa("Bio::EnsEMBL::Analysis");
...
sub LSF_out {
    my ( $self, $arg ) = @_;
    ( defined $arg ) && ( $self->{'_lsfout'} = $arg );
    $self->{'_lsfout'};
}

sub LSF_err {
    my ( $self, $arg ) = @_;
    ( defined $arg ) && ( $self->{'_lsferr'} = $arg );
    $self->{'_lsferr'};
}

sub retry_count {
    my ( $self, $arg ) = @_;
    ( defined $arg ) && ( $self->{'_retry_count'} = $arg );
    $self->{'_retry_count'};
}
...
=head2 flush_runs
  ...
  Function  : When there are a defined number of jobs ready to run (set in pipeconf or RuleManger) 
            : this submits them to lsf using the adaptor to connect to the database. 
            : If there is more than one job being submitted the last job is used for stdout/stderr
  Returntype: none 
...
#once this has been established for each queue jobs should be run on the last job id is got and the 
#bsub line constructed and submitted and the job status is set to submitted or failed depending on 
#whether the job has an lsfId
...
sub runInLSF {
    ...
    if ($autoupdate) {
        eval {
            ...
            $sic->store_inputId_class_analysis( $self->input_id, $self->class, $self->analysis );
        };
        if ( $err = $@ ) {
...
sub write_object_file {
    ...
    if ( defined($arg) ) {
        ...
        open( OUT, ">" . $self->input_object_file )
          || $self->throw( "Couldn't open object file " . $self->input_object_file );
        print( OUT $str );
        close(OUT);
    }
}
