...
my $max_jobs  = $::pipeConf{'maxjobs'} || 100; # max number of (pend) jobs
...
You cannot have two RuleManager's connecting to the same database.
...
while (1) {
	...
	#
	# FIXME: list_inputId_class_by_start_count() returns a list ordered by
	# number of entries per inputId/class. So if we get the list of inputId's
	# piecewise like this it will not get each inputId once only! Solutions:
	# get all inputId's at once, or remove the "ORDER by c" clause from
	# StateInfoContainer::list_inputId_class_by_start_count (put in to
	# 'prioritize' inputIds which needed most work on them.)
...
sub lsf_jobs {
    ...
    my $jobname = $LSF_params->{'jobname'};
    my $cmd;

    if (defined $jobname) {
	$cmd = "bjobs -p -J $jobname | grep PEND";
    }
    elsif (defined $queue) {
	$cmd = "bjobs -p -q $queue | grep PEND";
    }
    else {
	$cmd = "bjobs enspipe -p | grep PEND";
    }
