...
my $jobname   = $::pipeConf{'jobname'} || undef;
...
my $shuffle      = 1;       # Shuffle IDs before running
			    # Otherwise may be that IDs with most jobs done will
			    # be picked first
my @analysis;               # Only run these analysis ids
my %analysis;
my $submitted;
my $idlist;
my ($done, $once);
...
# really need a SIGTERM handler as well here.

$SIG{'ALRM'} = \&alarmhandler;
my $alarm  = 0;
my $wakeup = 120;   # period to check LSF queues; 0 to disable
...
while (1) {
    ...
    my @idNew;
    push @idNew, splice (@idList, rand @idList, 1) while @idList if $shuffle;
    @idList = @idNew;
    undef @idNew;
	...
	# SIG handling stuff here (i.e. once per 'ID' loop)

	# they are 'caught' in the handler subroutines but
	# it is only here they we do anything with them.
	# so if the script is doing something somewhere else
	# (getting IDs at the start of the while loop) or dozing,
	# we have to wait until it gets here to do anything...

	if ($alarm == 1) {
	    # alarm bell has been rung
	    $alarm = 0;
	    while (&lsf_jobs($LSF_params) >= $max_jobs) {
		sleep 300;
	    }
	    alarm $wakeup;
	}
