...
=head1 SYNOPSIS
  ...
  my $hsperategenscan = Bio::EnsEMBL::Pipeline::Runnable::HsperateGenscanPep->new ( 
                                                    -genomic    => $genomic,
                                                    -peptide    => $pep,
                                                    -database   => 'swir',
                                                    -threshold  => 1e-6,
  ...
  $hsperategenscan->run();
  $hsperategenscan->output();

=head1 DESCRIPTION

This object runs Bio::EnsEMBL::Pipeline::Runnable::Hsperate on peptides
constructed from assembling genscan predicted features to peptide
sequence. The resulting hsperate hits are written back as FeaturePairs.
...
package Bio::EnsEMBL::Pipeline::Runnable::HsperateGenscanPep;

use strict;
use Bio::EnsEMBL::Pipeline::RunnableI;
use Bio::EnsEMBL::Pipeline::Runnable::Hsperate;
...
=head2 new
    ...
    Function:   creates a Bio::EnsEMBL::Pipeline::Runnable::HsperateGenscanPep object
...
sub new {
  ...
  my ( $genomic,$transcripts,$program,$database,$threshold,$threshold_type,$options) = 
    $self->_rearrange ([qw(GENOMIC TRANSCRIPTS PROGRAM DATABASE THRESHOLD THRESHOLD_TYPE
                           OPTIONS)], @args);
  ...
  if (defined($transcripts) && (ref($transcripts) eq 'ARRAY')) {
    $self->transcripts($transcripts);
  } else {
    $self->throw("No transcripts input for $transcripts");
  }
...
=head2 run
    ...
    Title   :   run
    Usage   :   $self->run;
    Function:   Runs Bio::EnsEMBL::Pipeline::Runnable::HsperateGenscanPep->run
    Returns :   none
    Args    :   none
...
sub run {
  ...
  if (!defined($self->transcripts)) {
    $self->throw("No transcripts input");
  }
  ...
  my $runnable = new Bio::EnsEMBL::Pipeline::Runnable::Hsperate(
                                                         -queries   => \@peptides,
...
=head2 output
    ...
    Title   :   output
    Usage   :   $self->output();
    Function:   Runs Bio::EnsEMBL::Pipeline::Runnable::HsperateGenscanPep->output()
...
# and a position within the genscan predicted peptides. The hash is then matched
# against hsperate peptide hits to return a set of featurepairs of exons and hsperate 
# peptides
sub align_hits_to_contig {
    my ($self, @features) = @_;


    foreach my $trans ($self->transcripts) {
      my (%dna_align, @exon_aligns); #structure for tracking alignment variables
  ...
  #        print STDERR "exon\tgenscan\tsimilarity\t" . $exon->start . "\t" . $exon->end . "\t100\t" . $strand ."\t" . $exon->phase . "\t" . $trans->id . "\n";
          ...
          # (for debugging use this one) $ex_align {'name'}      = $trans->temporary_id;
      ...
      FP: foreach my $fp (@features) {
        next FP unless $fp->feature1->seqname eq $trans->temporary_id;

        print "Checking " . $fp . " start " . $fp->start . " end " . $fp->end . "\n";
...
# This function takes a hsperate peptide feature hit and a set of matching exons and
...
sub get_Sequence {
    ...
    if ($BIOPERLDB) {
      ...
      if ($@) {
        print STDERR ("Couldn't find sequence for [$id] in HsperateGenscanPep");
        return;
      }
...
sub transcripts {
    my($self,$transcript_ref) = @_;
    
    if (defined($transcript_ref)) {
      if (!(ref($transcript_ref) eq 'ARRAY')) {
        $self->throw("[$transcript_ref] is not a reference to an ARRAY");
      }
      $self->{'_transcripts'} = $transcript_ref;
      foreach my $transcript ($self->transcripts) {
        print "Got transcript " . $transcript . " ref " . ref($transcript) . "\n";
      }
    }
...
=head2 program
    ...
    Title   :   program
    Usage   :   $self->program('hsperate');
    Function:   Get/set for the flavour of hsperate to run
    Returns :   String
    Args    :   String
...
=head2 options
    ...
    Title   :   options
    Usage   :   $self->options('-H 150');
    Function:   Get/set for the options to pass to the hsperate command
    Returns :   string
    Args    :   string
