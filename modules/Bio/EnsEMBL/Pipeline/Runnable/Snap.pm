...
=head2 hmmfile
    ...
    Function:   Get/set method for the location of snap hmmfile
    Args    :   File path (optional)
...
=head2 parameters
    ...
    Function:   Get/set method for the location of snap parameters
    Args    :   File path (optional)
...
sub calculate_and_set_phases_new {
    my ($self) = @_;
    my $min_gene_length = 20;
    #Max low complexity allowed (in %)
    my $max_low_compl = 35;
  ...
  GENE: while ($i < scalar(@genes)) {
      ...
      my $tran = Bio::EnsEMBL::Pipeline::Tools::PredictionTranscriptFactory::fset2transcript_guess_phases($genes[$i],$self->slice);

      print STDERR "\nPeptide is " . $peptides[$i] . "\n";
      

      if ($tran) {
	  my $n_pep = $tran->translate->seq();
	  
	  print STDERR "\nNew pep is $n_pep\n";
	  
	  my $striped_pep = $n_pep;
	  
	  $striped_pep =~ s/X//g;
	  my $compl = (length($peptide) - length($striped_pep))*100 / length($peptide); 
	  if ($compl>=$max_low_compl) { 
	      
	      print STDERR "peptide $i ($n_pep) has too much low complexity ($compl %)\n";
	      $i++;
	      next GENE;
	  } 
