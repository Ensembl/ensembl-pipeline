...
sub calculate_and_set_phases_new {
  ...
  GENE: while ($i < scalar(@genes)) {
      ...
      print STDERR "peptide length ".length($peptide)."\n";
      ...
      foreach my $tran (@newtran) {
        print STDERR $tran->start." ".$tran->end." ".@{$tran->get_all_Exons}
          ." transcript details\n";
        my $temp_tran = $tran->translate->seq;
        print STDERR "peptide length ".length($temp_tran)."\n";
