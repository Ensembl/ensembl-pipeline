...
=head1 DESCRIPTION
    ...
    permitted for this EST. If depth is not set, it won't try to 
    take the best 'd' ests in the cluster.
 ...
 EXON:
  ...
  foreach my $cluster ( @sorted_clusters ){
      my @exons = sort { $self->_exon_score($b) <=> $self->_exon_score($a)
			     or
				 $self->_perc_id($b) <=> $self->_perc_id($a) 
				 } 
