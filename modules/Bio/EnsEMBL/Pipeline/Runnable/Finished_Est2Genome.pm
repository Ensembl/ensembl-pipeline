...
sub run {
    ...
    my $est2genome_command =
      "/usr/local/ensembl/bin/est2genome  -reverse -genome $genfile -est $estfile -space 500000 -out stdout 2>&1 |";

    #print STDERR "running for " . $estseq->display_id . "\n";

    print( STDERR "Running command $est2genome_command\n" );
    open( ESTGENOME, $est2genome_command ) or confess("Can't open pipe from '$est2genome_command' : $!");
    ...
    while (<ESTGENOME>) {
        ...
        if ( $_ =~ /^Segment/ ) {    # We only care about Segments
            ...
            if ( $estOrientation == -1 ) {
                ( $gen_start, $gen_end, $est_start, $est_end ) = ( $gen_end, $gen_start, $est_end, $est_start );
            }
        ...
        elsif ( $_ =~ /Segmentation fault|ERROR/ ) {
            $self->warn("Error running est2genome\n");
            close(ESTGENOME) or $self->warn("problem running est2genome: exit $?\n");
            return (0);
        }

    }
    if ( !close(ESTGENOME) ) {
        $self->throw("problem running est2genome: exit $?\n");
    }
...
sub _create_FeaturePair {
    my (
        $self,  $f1score, $f1percent_id, $f1start,  $f1end,    $f1id,    $f2start,
        $f2end, $f2id,    $f1source,     $f1strand, $f2strand, $f1primary
      )
      = @_;
