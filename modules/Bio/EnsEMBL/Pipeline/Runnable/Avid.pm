...
$workdir     = an optional string containing the location of the working-directory (default: /tmp/)
$file_prefix = an optional array of strings used as file_prefixes for the written output-files
...
sub new {
  ...
  print "Passed options: ".$self->options."\n" if $self->verbose;

  ########## FILE_PREFIX ####################

  if (ref ($file_prefix) eq "ARRAY") {
