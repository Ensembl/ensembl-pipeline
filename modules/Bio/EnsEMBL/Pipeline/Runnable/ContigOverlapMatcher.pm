...
sub run {
    ...
    eval{
        mkdir($tmp_dir, 0777) or die "Can't mkdir('$tmp_dir') : $!";
        my $seq_file = "$tmp_dir/contig.seq";
        $self->_write_seqs_to_file($seq_file);
        my $command = "cd $tmp_dir; phrap.longreads -new_ace -default_qual 90 -minmatch 30 -maxmatch 30 $seq_file >/dev/null 2>&1";
        system($command) == 0
...
sub _make_sorted_spans {
    ...
    for (my $i = 0; $i < @base_segments; $i++) {
        ...
        # the matching position is the coordinate minus 1.  ($next won't
        # exist if this is the last contig in the assembly.)
        if (my $next = $base_segments[$i+1]) {
            my( $j_name, $j_start ) = @{$next}[2,3];
            $j_start--;
            if ($j_start < 1) {
                $self->throw("Bad coordinate (4th element) in: (".
                    join(',', map "'$_'", @$next) .")");
            }
            $span{$name}->[5] = $j_name;
            $span{$name}->[6] = $j_start;
    ...
    return sort {$a->[0] <=> $b->[0]} values %span;
}
    ...
    sub _make_overlap_helpers {
        ...
        for (my $i = 0; $i < (@span - 1); $i++) {
            ...
            my $ovrlp = Bio::EnsEMBL::ContigOverlap->new(
                '-contiga'      => $self->get_Contig($a_name),
                '-contigb'      => $self->get_Contig($b_name),
                '-positiona'    => $a_pos,
                '-positionb'    => $b_pos,
                '-overlap_tpe'  => $type,
                '-source'       => 'phrap',
