...
package Bio::EnsEMBL::Pipeline::RunnableDB::MiniGenewise;
...
sub run {
    ...
    print STDERR "Reverse is $reverse\n";
	...
	$tmpf->score(0);
	$tmpf->source_tag('genewise');
	$tmpf->primary_tag('similarity');

	foreach my $subf ($tmpf->sub_SeqFeature) {
	    $tmpf->analysis($subf->analysis);
	    $subf->seqname($self->input_id);
	    $subf->id($self->input_id);
	    $subf->source_tag("genewise");
	    $subf->primary_tag("similarity");
	    $subf->strand($subf->strand);
	}
...
sub output {
    my ($self) = @_;
    print STDERR "Runnable " . $self->runnable . "\n";
    unless (defined($self->runnable))
    {
...
sub get_Sequences {
	...
	my $id = $pair->hseqname;
	print STDERR $id . " " . $pair->analysis->db . "\n";
	if ($pair->analysis->db eq "swir") {
            eval {
...
sub make_blast_db {
    ...
    my $status = system("setdb $blastfile");
    print (STDERR "Status from setdb $status\n");
...
sub run_blast {
    ...
    my $command  = "blastx $db $seqfile B=500 -hspmax 1000 2> /dev/null |MSPcrunch -d - >  $blastout";
