#
# Ensembl module for Bio::EnsEMBL::Pipeline::RunnableDB::Contig_TargettedE2G.pm
#
...
Bio::EnsEMBL::Pipeline::RunnableDB::Contig_TargettedGeneE2G
...
Like TargettedGeneE2G, uses genewise and est_genome to build genes
from protein/cdna pairs and genomic locations.

Unlike TargettedGeneE2G, runs on RawContigs rather than
VirtualContigs, making it suitable for organisms where we don\'t yet
have a real golden path.
...
package Bio::EnsEMBL::Pipeline::RunnableDB::Contig_TargettedGeneE2G;
...
sub fetch_input{
  ...
  # input format: AC079574.1.212019.234257:19380,20303:Q9R225:98.1
  # or AC079574.1.212019.234257:19380,20303:Q9R225: if no cDNA
  ...
  # genewise runnable
  # repmasking?
  my $r = Bio::EnsEMBL::Pipeline::Runnable::BlastMiniGenewise->new( '-genomic'    => $genseq,
								    '-ids'        => [ $protein_id ] ,
  ...
  if($@) {
    ...
    # repmasking?
    my $e2g = new Bio::EnsEMBL::Pipeline::Runnable::ExonerateMiniEst2Genome('-genomic'    => $genseq, 
									    '-queryseq'   => $cdnafile,
...
sub run {
   ...
   # check that genes translate
   my @checked = $self->check_all_genes();
   $self->{'_output'} = \@checked;
}
...
sub check_gw_genes{
  my ($self, @genes) = @_;
  my $threshold = 50; # 50 for contig based, as we expect to have broken up proteins
  ...
  foreach my $gene(@genes) {
    ...
    if(!$covered){
      my $msg = "rejecting " . $gene->id . " because < $threshold% coverage of parent protein\n";
      $self->warn($msg);
      next GENE;
    };
...
sub _make_newtranscripts {
  ...
  my @newtrans  = ();
  my $threshold = 50; # lower bound cutoff for protein coverage
 ...
 E2G:
	...
	foreach my $ex($newtranscript->each_Exon){
	  $ex->attach_seq($self->vc->primary_seq);
...
=head2 check_all_genes

 Title   : check_all_genes
 Usage   :
 Function: 
 Example :
 Returns : 
...
GENE:  foreach my $gene (@genes) {
    print STDERR "about to check translation " . $gene->id . "\n";
    ...
    if($gene->type eq 'TGE_gw' || $gene->type eq 'combined_gw_e2g'){
      ...
      if(!$translates){
	my $msg = "discarding " . $gene->id . " - translation has stop codons\n";
	$self->warn($msg);
	next GENE;
      }
    }
...
print STDERR "transcript: $transcript\n";
  ...
  if((!defined $tseq) || ($@)){
    my $msg = "problem translating " . $transcript->id . " :\n$@\n";
    $self->warn($msg);
    return 0;
  }
...
sub _check_coverage{
  ...
  foreach my $exon($gw_tran[0]->each_Exon) {
    $pstart = 0;
    $pend   = 0;
...
sub validate_gene{
  ...
  if(scalar(@transcripts) != 1) {
    my $msg = "Rejecting " . $gene->id . " - should have one transcript, not " . scalar(@transcripts) . "\n";
    $self->warn($msg);
    return 0;
  }
  ...
  foreach my $transcript(@transcripts){
	...
	my $msg = "Rejecting " . $gene->id . " because of invalid exon\n";
	$self->warn($msg);
	return 0;
      }
    }
...
sub validate_exon{
  ...
  if($exon->start < 0){
    my $msg = "rejecting " . $exon->id . ",  start < 0 : " . $exon->start . "\n";
    $self->warn($msg);
    return 0;
  }

  elsif($exon->start > $exon->end){
    my $msg = "rejecting " . $exon->id . ",  start > end : " . $exon->start . " > " . $exon->end . "\n";
    $self->warn($msg);
    return 0;
  }
