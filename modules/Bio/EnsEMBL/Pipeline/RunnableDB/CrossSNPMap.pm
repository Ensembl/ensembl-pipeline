...
sub _pre_run {
  ...
  foreach my $contig(@contigs ) {
    ...
    $id_hash{$id}=$contig->name;
    $self->_acc($contig->name);
...
sub write_output{ 
  ...
  if (@snp_fps==0 or @snp_fps >10 ) {
	...
	if ($fp[0]->strand ==1 and $fp[1]->strand ==1) {
	  $start = $fp[0]->start;
	  $end = $fp[1]->start;
	  print "here 3 ",$fp[0]->strand, "\t",$fp[0]->start,"\t",$fp[1]->start,"\n";
	}
	elsif ($fp[0]->strand ==-1 and $fp[1]->strand ==-1)  {
	  $start = $fp[0]->end;
	  $end = $fp[1]->end;
	  print "here 4 ",$fp[0]->strand,"\t",$fp[0]->end,"\t",$fp[1]->end,"\t\n";
	}
	else {
