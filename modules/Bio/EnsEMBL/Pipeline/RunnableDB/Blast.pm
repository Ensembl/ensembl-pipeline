...
=head2 new
    ...
    Args    :   -dbobj:     A Bio::EnsEMBL::DB::Obj, 
                ...
                -args:      Args for blast (optional),
                -blast:     blast executable ,
                -db:        database used for blast. 
...
sub new {
    ...
    my ( $dbobj, $input_id, $args, $blast, $db) = 
            $self->_rearrange (['DBOBJ', 'INPUT_ID', 'ARGS', 'BLAST', 'DB'], @args);
    ...
    $self->runnable(    -runnable => 'Bio::EnsEMBL::Pipeline::Runnable::Blast',
                        -blast    => $blast, 
                        -db       => $db, 
                        -args     => $args);
    return $self;
...
sub runnable {
    my ($self, @args) = @_;
    my ($runnable, $blast, $db, $args) = 
            $self->_rearrange (['RUNNABLE', 'BLAST', 'DB', 'ARGS'], @args);
    if ($runnable)
    {
