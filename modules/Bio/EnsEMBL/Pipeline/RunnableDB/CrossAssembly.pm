...
package Bio::EnsEMBL::Pipeline::RunnableDB::CrossAssembly;
...
sub fetch_input {
    ...
    #get the compara dbadaptor from the current ensembl dbadaptor
    my $newadp = $self->dbobj->get_ComparaDBAdaptor();
    my $gadp = $newadp->get_GenomeDBAdaptor();
...
sub output {
   ...
   if (defined $self->{'output'}) {
      ...
      #need this else was getting this error PERL error about cannot use an uninitialised ARRAY value..
      return ();
   }
}
...
sub _greedy_filter {
  ...
  my $ref_strand;
  NEXT: foreach my $fp (@features) {
    # tania: need to filter hits where you hit scaffold against itself, 
    # coz you don't want the exact matches (same coords), only want the  
    # genome/scaffold is duplicated 
    
    if ((($fp->hstart == $fp->start) && ($fp->hend == $fp->end))){
       # print "Hstart: ", $fp->hstart, "hend:",  $fp->hend, " end", $fp->end, "start", $fp->start, "\n";
        print STDERR "getting out of this fp coz it's just matching itself, sans utile\n";
        next NEXT; 
    } 
...
sub write_output {
  ...
  #tania, getting the right adaptor
  my $db = $self->dbobj->get_ComparaDBAdaptor();
  ...
  #tania: need this again to check if features exist, else get errors 
  if (! scalar @features){ return undef; }
