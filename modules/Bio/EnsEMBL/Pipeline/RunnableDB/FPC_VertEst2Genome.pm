...
Bio::EnsEMBL::Pipeline::RunnableDB::FPC_VertEst2Genome

=head1 SYNOPSIS

    my $obj = Bio::EnsEMBL::Pipeline::RunnableDB::FPC_VertEst2Genome->new(
					     -dbobj     => $db,
					     -input_id  => $id
                                             );
    $obj->fetch_input
...
package Bio::EnsEMBL::Pipeline::RunnableDB::FPC_VertEst2Genome;
...
sub fetch_input {
 ...
 foreach my $contig (@contig){
   ...
   my @new_features = $self->_process_features($contig,@features);

   if(scalar(@new_features))
     {
       # set up the runnables
       $self->_prepare_runnables($contig, @new_features);
     }
   else { print STDERR "No features to be analysed for " . $contig->id()  . "\n"; }
 }
}
...
=head2 _process_features
    ...
    Usage   :   $self->_process_features($genseq,@features)
...
sub _make_exons {
  ...
  my $analysis_obj = Bio::EnsEMBL::Analysis->new
    (
     ...
     -program         => 'est_genome',
     -program_version => '1',
     -gff_source      => 'FPC_vert_est2genome',
