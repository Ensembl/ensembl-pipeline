...
sub run {
   ...
   # will just be the same as for TargettedGeneDNA
...
sub convert_e2g_output {
  ...
  my @tmpf = $self->e2g_runnable->output;
  
  # ought really to standardise how E2G,Exonerate,Genewise etc return their 
  # features. Then could have a genemaking method bag inheritable by all 
  # of them.
  ...
  # fix strands - probably should be done in Est2Genome, but that may well break 
  # other runnableDBs ....
  ...
  my @genes = $self->make_genes($count,$time,$self->e2g_runnable);
  
  my @remapped = $self->remap_genes($self->e2g_runnable,@genes);  
  ...
  if (!defined($self->{_output})) {
    ...
    print STDERR "****e2g genes: " . scalar(@remapped) . "\n";
...
sub make_genes {
  ...
  if ($runnable->isa("Bio::EnsEMBL::Pipeline::Runnable::BlastMiniGenewise")){
    $genetype = "test_TGE_gw";
  }
  elsif ($runnable->isa("Bio::EnsEMBL::Pipeline::Runnable::Est2Genome")){
    $genetype = "test_TGE_e2g";
  }
  else{
