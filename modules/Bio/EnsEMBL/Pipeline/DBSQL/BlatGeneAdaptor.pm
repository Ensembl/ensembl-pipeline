...
my $genes = $blat_genes->fetch_all_by_Slice_and_type($slice);
...
sub store {
  ...
  my $storable_gene = $self->convert_to_ascii_codes($gene_dump);

  my $sql = "INSERT INTO blat_gene_dump(chr_name, gene_start, gene_end, dumped_gene, type) " .
            "VALUES (?, ?, ?, ?, ?)";

  my $sth = $self->prepare($sql);

  $sth->execute($gene->chr_name, $gene->start, $gene->end, $storable_gene, $gene->type);

  return $sth->{'mysql_insertid'};
}


sub convert_to_ascii_codes {
  my ($self, $string) = @_;
  ...
  foreach my $letter (@letters) {
    $converted .= ord($letter) . ":";
  }

  return $converted;
}

sub convert_from_ascii_codes {
  my ($self, $string) = @_;
...
sub get_genes_by_Slice_and_type {
  ...
  my $sql = q{SELECT blat_gene_dump_id, dumped_gene, mapped_status
	      FROM blat_gene_dump
	      WHERE chr_name = ?
	      AND gene_end >= ?
	      AND gene_start <= ?
 ...
 DUMPED_GENE:
  while (my ($dumped_gene_id, $retrieved_gene, $mapped_status) = $sth->fetchrow_array) {

    next DUMPED_GENE unless ($mapped_status);

    my $gene_dump = $self->convert_from_ascii_codes($retrieved_gene);
    ...
    if ($gene_dbID) {
      my $update_sql = qq{UPDATE blat_gene_dump 
			  SET mapped_status = $gene_dbID
			  WHERE blat_gene_dump_id = $dumped_gene_id};

      my $update_sth = $self->db->prepare($update_sql);
      $update_sth->execute;
