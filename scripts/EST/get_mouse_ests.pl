...
=head1 DESCRIPTION
  ...
  gets mouse ESTs from dbEST and polyT/polyA clips them ready for exonerate
...
if(!defined $estfile    ||
  ...
  print  "USAGE: get_mouse_ests.pl -estfile estfile -outfile outfile\n";
  exit(1);
}
...
while (<EST>){
  ...
  next SEQFETCH unless $description =~ /Mus musculus cDNA/;
  if($description =~ /similar to .* Mus musculus cDNA/){
    next SEQFETCH unless $description =~ /Mus musculus.*similar to.*Mus musculus cDNA/;
