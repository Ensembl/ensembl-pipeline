...
my $module_cross     = 'Bio::EnsEMBL::DBSQL::CrossMatchDBAdaptor';
my $module_snp = 'Bio::EnsEMBL::ExternalData::SNPSQL::DBAdaptor';

my $locator_cross = "$module_cross/host=$host_snp;port=$port;dbname=$dbname_snp;user=ensro";
my $locator_snp = "$module_snp/host=$host_snp;port=$port;dbname=$dbname_snp;user=ensro";

print STDERR "Using $locator_cross for crossmatch db\n";
my $crossdb =  Bio::EnsEMBL::DBLoader->new($locator_cross);
my $db_snp =  Bio::EnsEMBL::DBLoader->new($locator_snp);
my $db = $crossdb->new_dbobj;#dbhandle defined in snp120 database
...
my ($file_num,$input,$diff,$seq_file,$ssaha_out);
...
if ($ARGV[0] and $ARGV[1]) {
  ...
  $file_num = $ARGV[0];
  open FILE, "/work4/yuan/SNP_MAP/NO_CLONE_$file_num" || die "Can not open file: $! \n";
  #open FILE, "/work4/yuan/OUT1/NO_CROSS_MATCH$file_num" || die "Can not open file: $! \n";
  open DIFF, ">/work4/yuan/SNP_MAP/SNP_SSAHA_FROM_NO_CLONE_$file_num" || die "Can not open FINAL: $! \n";
  ...
  my $seq_file =  "/nfs/acari/yuan/ensembl/test/SNP_SSAHA_NO_CROSS_MATCH_seq_$file_num";
  my $ssaha_out = "/nfs/acari/yuan/ensembl/test/NO_CROSS_MATCH_seq_$file_num";
}
...
if (!(-e $ssaha_out)) {
  ...
  foreach my $refsnpid (sort numeric keys %no_clone) {
    my $info = $db_snp->get_snp_info_by_refsnpid($refsnpid);
    if ($info) {
...
if (!(-e $ssaha_out) and -e $seq_file) {
  open IN, "cat $seq_file | /usr/local/badger/bin/ssahaClient tcs1a 50000 20 0 0 0 DNA 10000 3 size |" ||  die "OPEN SSAHA failed :$!\n";
}
elsif (-e $ssaha_out) {
...
foreach my $refsnpid (keys %find) {
  ...
  foreach my $contig (@contigs) {
    if (!$uniq_contig{$contig}) {
      $uniq_contig{$contig}=1;
      if (!$db) {
	$db = $crossmap->dbobj->new_dbobj; 
      }
      my $rc;
      eval {
  ...
  my $done;
  if ($rec{$refsnpid}) {
    ($done) = $crossmap->check_clone_in_GP($rec{$refsnpid},@raw_contigs);
  }
  else {
    my $info = $db_snp->get_snp_info_by_refsnpid($refsnpid);
    if ($info) {
      print "refsnpid is $refsnpid and info is $info\n";
      ($done) = $crossmap->check_clone_in_GP($info,@raw_contigs);
    }
  }
  if ($done =~ /diff/) {
    $crossmap->run;
