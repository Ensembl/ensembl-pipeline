...
my ($file,$id_file,$first,$file_num,$first,$input,$search_file,$start_internal_id,$diff,$seq_file,$ssaha_out,$feature_out,$mouse,%inseq);
...
####if use $ARGV[0] eq "file", the file contains a list of flanking sequences to be mapped###
...
###ssaha_zn_old is quicker and more strict, but will miss some hits. I use this first, then use ssaha with no_matched ones
else { 
