...
for( my $i_species = 0; $i_species<=$#target_genomes; $i_species++ ) {
  ...
  # start with an empty seq of the same length as the focus seq
  $target_seq[$i_species] = "~" x length( $focus_seq );
 ...
 FRAG:
  ...
  ALIGN:
	...
	$alignments_found = 1; 
      ...
      substr( $target_seq[$i_species],$p_start - 1,length( $aligned_string ), $aligned_string );
