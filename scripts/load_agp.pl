...
my $assembled_cs = $csa->fetch_by_name( $assembled_name, $assembled_version );
my $component_cs = $csa->fetch_by_name( $component_name, $component_version );
...
LINE: while (<FH>) {
    ...
    my (
        $a_name_init, $a_start, $a_end, $ordinal, $type,
        $c_name,      $c_start, $c_end, $ori
    ) = split;
    ...
    if ( !$assembled_ids{$a_name} ) {
        ...
        my $a_piece =
          $sa->fetch_by_region( $assembled_cs->name, $a_name, undef, undef,
            undef, $assembled_cs->version );
        if ( !$a_piece ) {
    ...
    if ( $component_ids{$c_name} ) {
        ...
        my $c_piece =
          $sa->fetch_by_region( $component_cs->name, $c_name, undef, undef,
            undef, $component_cs->version );
        if ( !$c_piece ) {
    ...
    &insert_agp_line( $a_id, $a_start, $a_end, $c_id, $c_start, $c_end, $strand,
        $db );
}
...
sub insert_agp_line {
    my ( $chr_id, $chr_start, $chr_end, $contig, $contig_start, $contig_end,
        $contig_ori, $db )
      = @_;
...
#print STDERR "trying to insert into ".$chr_id." ".$chr_start." ".$chr_end."\n";
    ...
    my $sql =
"insert ignore into assembly(asm_seq_region_id, asm_start, asm_end, cmp_seq_region_id, cmp_start, cmp_end, ori) values(?, ?, ?, ?, ?, ?, ?)";
