...
###########################################
# set up an ensembl database for c.elegans with
# data from the wormbase
#
# semi-automated procedure, un-comment desired steps...
#
...
# 7-run checks (translation)
# 8-prepare raw computes with pipeline
#
###########################################
...
my @steps = qw(sequence_store.pl gene_store.pl operon_store.pl expr_store.pl rnai_store.pl pseudo_store.pl);
...
sub dump_translations{
  ...
  $cmd = "perl ".$cvsDIR."ensembl-pipeline/scripts/protein_pipeline/dump_translations.pl ".
         "-dbname $WB_DBNAME -dbport $WB_DBPORT -dbhost $WB_DBHOST -dbuser $WB_DBUSER -dbpass $WB_DBPASS -db_id 1 > ".
	 $workDIR."peptide.fa";
  $status += system($cmd);
...
sub protein_X_mapping{
  ...
  #Bio::EnsEMBL::Pipeline::Config::GeneBuild/GB_PFASTA --> mappping_conf/total_known_fa
  ...
  #might need specific version (0.7.1) in mapping.conf
  #-->perl ensembl/misc-scripts/protein_match/exonerate_wrapper.pl -d
  #or as a bsub-job
  #select big machines
  #-->bhosts -R 'select[mem>4000 && model==ES451250] rusage[mem=4000]'
  #run with output into file
  #-->bsub -q bigmem -R 'select[mem>4000 && model==ES451250] rusage[mem=4000]'
  #   -o /nfs/acari/fsk/projects/wormbase/files/exonerate.out perl 
  #   /nfs/acari/fsk/cvs_checkout/old/ensembl/misc-scripts/protein_match/exonerate_wrapper.pl -d -v

  #write xrefs into database (tables: xref, object_xref, identity_xref, external_synonym)
  #SWISSPROT / SPTREMBL might have to be changed to Uniprot/SWISSPROT and Uniprot/SPTREMBL !
