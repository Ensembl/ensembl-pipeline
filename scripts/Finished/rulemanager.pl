...
$ENV{BLASTDB} = '/data/blastdb/Supported';
...
my $sanity = Bio::EnsEMBL::Pipeline::Utils::PipelineSanityChecks->new(-DB => $db,);
...
setup_pipeline(
    \%analyses_to_run,      \%analyses_to_skip,               $all_rules,
    \%accumulator_analyses, \%always_incomplete_accumulators, $ids_to_run,
    $ids_to_skip,           \@types_to_run,                   \@types_to_skip,
...
LOOP: while (1) {
    ...
    if ($reread_input_ids) {
        $rulemanager->input_id_setup($ids_to_run, $ids_to_skip, \@types_to_run, \@types_to_skip, \@starts_from);
    }
    if ($reread_rules) {
        $rulemanager->empty_rules;
        $all_rules = $rulemanager->rules;
        $rulemanager->rules_setup(\%analyses_to_run, \%analyses_to_skip, $all_rules, \%accumulator_analyses,
                    ...
                    && !exists($completed_accumulator_analyses{$logic_name}))
                {
                    ...
                    if (
                        $rulemanager->can_job_run(
                            'ACCUMULATOR', $accumulator_analyses{$logic_name},
                      ...
                      )
                    {
                        ...
                        print "Accumulator type analysis $logic_name " . "conditions unsatisfied\n"
                          if $verbose;
                    }
                    else {
                        print "Accumulator type analysis $logic_name " . "already run\n"
                          if $verbose;
                    }
                }
            }
...
sub update_analysis_dbversion {
    ...
    foreach my $ana (@analysis) {
        ...
        # if ($mod && ($mod eq 'Blast' || $mod =~ /Exonerate/ || $mod =~ /Halfwise/)) {
            $a_hash->{$ln} = $ana;
...
sub useage {
    ...
    print "ensembl-pipeline/scripts/rulemanager.pl is the main script used " . "to run the pipeline\n\n";
    print "Everytime you run the rulemanager you must pass in the database " . "options\n\n";
