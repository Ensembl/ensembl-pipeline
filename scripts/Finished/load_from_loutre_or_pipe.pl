#!/usr/bin/env perl
# $Source: /tmp/ENSCOPY-ENSEMBL-PIPELINE/scripts/Finished/load_from_loutre_or_pipe.pl,v $
# $Revision: 1.10 $
...
#
{
    print STDOUT "Getting Sequence set @sets from source database: $sname ($shost:$sport)\n";
    my $contigs_sth = $source_dbc->prepare(q{
        SELECT a.asm_start
          , a.asm_end
          , a.cmp_start
          , a.cmp_end
    ...
    my $description_sth = $source_dbc->prepare(q{
