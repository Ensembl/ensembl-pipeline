...
sub DB_setup{
  ...
  eval{
    ...
    $status += system($cmd);
    print ".";
    $status += system("mysql -h$WRITEHOST -P$WRITEPORT -u$WRITEUSER -p$pass $WRITENAME -e\"load data infile \'$CVSDIR/ensembl/misc-scripts/external_db/external_dbs.txt\' into table external_db;\"");
    print ".";
