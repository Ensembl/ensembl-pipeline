...
if ($comm eq "setup"){
  ...
  $cmd = "bsub -q normal -R model=IBMBC2800 -o $LC_workDIR/assembly/load_agp.log ".
 ...
 #insert a break or sleep here to wait for bsub
  ...
  print STDERR "\n>> Setting toplevel...\n";
  $cmd = "bsub -q normal -R model=IBMBC2800 -o $LC_workDIR/assembly/set_toplevel.log perl  ".
...
sub config_setup{
   ...
   system ("mkdir -p $ensconfDIR/Bio/EnsEMBL/Analysis/Config")
   	   && warn "\nProblem with making Analysis dirs in config setup\n";

   system ("mkdir -p $ensconfDIR/pipe_conf")
   	   && warn "\nProblem with making pipe_conf in config setup\n";

   system ("cp $LC_cvsDIR/ensembl-personal/ba1/armadillo1/generic_config/Bio/EnsEMBL/Pipeline/Config/* $ensconfDIR/Bio/EnsEMBL/Pipeline/Config/");
   
   foreach my $dir (@pipe_dirs){
       system ("cp $LC_cvsDIR/ensembl-personal/ba1/armadillo1/generic_config/Bio/EnsEMBL/Pipeline/Config/$dir/* $ensconfDIR/Bio/EnsEMBL/Pipeline/Config/$dir/");
   }

   system ("cp $LC_cvsDIR/ensembl-personal/ba1/armadillo1/generic_config/Bio/EnsEMBL/Analysis/Config/* $ensconfDIR/Bio/EnsEMBL/Analysis/Config/");
   system ("cp $LC_cvsDIR/ensembl-personal/ba1/armadillo1/generic_config/pipe_conf/* $ensconfDIR/pipe_conf/");
